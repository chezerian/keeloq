...
key = numpy.arange(data_points).astype(numpy.uint64)
ciph = numpy.arange(data_points).astype(numpy.uint32)
plain_result = numpy.empty_like(ciph)

# Speed in normal CPU usage
time1 = time()
plain_temp = (key+ciph) # adds each element in a to its corresponding element in b
plain_temp = plain_temp * plain_temp # element-wise multiplication
plain_result =  plain_temp * (key/2) # element-wise half a and multiply
time2 = time()
	...
	for device in platform.get_devices():
		...
		prg = cl.Program(ctx, """
			...
			__global const unsigned int *ciph, __global unsigned int *plain)
			{
						...
						plain_temp = key_temp+ciph_temp; //  sum of my elements
						plain_temp = plain_temp * plain_temp;  // product of sums
						plain_temp = plain_temp * (key_temp/2); // times 1/2 my a
